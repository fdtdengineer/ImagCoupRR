...
if __name__ == "__main__":
    ...
    delta = 0.0
    npr_Delta = np.array([1,2.4,4])*0.01 + 1#+10
    ...
    npr_kappa = np.linspace(0, 0.02, 21)
    ...
    for i, kappa in enumerate(npr_kappa):  
        ...
        for j, eig in enumerate(eigval):
            ...
            arg = np.array([np.abs(vec[0]), np.abs(vec[1]), np.angle(vec[1]), np.abs(vec[2]), np.angle(vec[2])])
