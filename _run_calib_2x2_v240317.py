...
if __name__ == "__main__":
    ...
    delta = 0.0
    npr_Delta = np.array([1,2])
    
    n = npr_Delta.shape[0]
    npr_eta = np.ones(n)*gain
    #kappa = 0.5 #
    theta = np.pi*(1-1e-3*2)#0.2*np.pi #
    kappa2 = 0 #
    theta2 = 0 #

    #npr_kappa = np.linspace(0, 0.5, 41)
    npr_kappa = np.linspace(0.2, 0.3, 41)
    ...
    def loss(H, arg, beta=gs):
        ...
        x_0 = arg[2] #+ 1.j * arg[2]
        x_1 = arg[3] *np.exp(1.j * arg[4])
        x = np.array([x_0, x_1])
        norm = np.linalg.norm(x)
        x /= max(norm,1e-10)
        #v = omega* x - H @ x + arg[1]*1.j* (beta * np.conj(x) @ x) * x
        v = omega* x - H @ x + arg[1]*1.j* (beta * np.abs(x)**2) * x        
        loss = np.conj(v).T @ v
        return loss.real
        
    npr_eig = np.zeros((npr_kappa.shape[0], n), dtype=float)
    npr_eig_skp = np.zeros((npr_kappa.shape[0], n), dtype=float)
    ...
    for j in range(n):
        ax.scatter(npr_kappa, npr_eig_skp[:,j], label=f"$\omega_{j}$", color="white",edgecolors="blue")
